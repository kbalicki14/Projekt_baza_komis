...
<main>
   ...
   <h1 style="color:green">
    ...
    <input id="reserve" class='button' type='submit' name='reserve' value='Zarezerwuj'>
    </form>
    <?php
    ...
    if (isset($_POST['reserve'])){
        if ($vehicle->sold =='y'||$vehicle->sold =='r' )
        {
            printf("Ten samochód został już zarezerwowany");
        }
        else{
    $sold = "reserve";
    ...
    UPDATE vehicle
    SET vehicle.sold = 'r'
    WHERE vehicle.id_vehicle = $id ");
    	if($query){
						echo "Zarezerwowano samochód";
					}else {
						echo "Nie udało się zarezerwować samochodu";
					}}
    $conn->close();
}?>
        <?php endif; ?>
